...
function EA.GetAmbushState()
    ...
    local getCampExitGrace = EA and EA["EA_GetCampExitAmbushGraceState"]
    local service = EA_GetAuthoredAmbushService()
    ...
    local campExitGraceActive = false
    local campExitGraceRemainingMs = 0
    ...
    if hostCharacter and hostCharacter ~= "" then
        ...
        if type(getCampExitGrace) == "function" then
            local okCampExit, campExitState = pcall(getCampExitGrace, hostCharacter)
            if okCampExit and type(campExitState) == "table" then
                campExitGraceActive = campExitState.active == true
                campExitGraceRemainingMs = math.max(0, math.floor(tonumber(campExitState.remainingMs) or 0))
            end
        end
    end
    ...
    return {
        ...
        campExitGraceActive = campExitGraceActive,
        campExitGraceRemainingMs = campExitGraceRemainingMs,
    }
end
