...
local function EA_WriteMCMSettingValue(id, value)
    if type(id) ~= "string" or id == "" then
        return false
    end
    local registerPresetSyncWrite = EA and EA["EA_RegisterPresetSyncWrite"]
    if type(registerPresetSyncWrite) == "function" then
        pcall(registerPresetSyncWrite, id, value)
    end

    local saved = false
    if MCM then
        if type(MCM.Set) == "function" then
            local okSet = pcall(MCM.Set, id, value, ModuleUUID, true)
            saved = saved or okSet == true
        end
        if type(MCM.SetSetting) == "function" then
            local okSetSetting = pcall(MCM.SetSetting, id, value, ModuleUUID, true)
            saved = saved or okSetSetting == true
        end
    end
    return saved
end
...
local function EA_PersistNormalizedPresetToMCMIfNeeded(rawPresetValue)
    ...
    return EA_WriteMCMSettingValue("MCM_DifficultyPreset", normalizedPreset)
end
...
local function EA_MigrateSpawnPlacementModeToOOSIfNeeded()
    ...
    local savedToMCM = EA_WriteMCMSettingValue("MCM_SpawnPlacementMode", "CREATE_OOS_ONLY")
...
EA_GetRuntimeSetting = function(id)
...
EA_ApplyRuntimeSettingsBatch = function(entries, opts)
...
if Ext and Ext.ModEvents and Ext.ModEvents.BG3MCM then
    ...
    if id == "MCM_AdvancedMode" then
        ...
        if (not suppressPresetCustom) and EA_ShouldMarkPresetCustom(id) and EA_TryMarkPresetCustom(id, "mcm_saved") then
            local basePreset = EA_NormalizePresetSettingForConfig(
                "MCM_CustomBasePreset",
                EA_GetRuntimeSetting("MCM_CustomBasePreset"),
                "Marked"
            )
            EA_WriteMCMSettingValue("MCM_DifficultyPreset", "CUSTOM")
            EA_WriteMCMSettingValue("MCM_CustomBasePreset", basePreset)
            effectiveChanged = true
        end
