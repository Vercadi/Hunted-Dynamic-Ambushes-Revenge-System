...
local function EA_PrintSupportAmbushState(character, label)
    local api = EA_GetAuthoredApiProbeApi()
    local state = nil
    local stateFn = type(api.rootState) == "function" and api.rootState or api.apiState
    if type(stateFn) == "function" then
        local okState, outState = pcall(stateFn)
        if okState and type(outState) == "table" then
            state = outState
        end
    end
    ...
    local rawRegion = ""
    local canonicalRegion = tostring(state.activeRegion or "")
    local regionFn = EA and EA["EA_GetRegionForCharacter"]
    if type(regionFn) == "function" and character and character ~= "" then
        local okRegion, canonical, raw = pcall(regionFn, character)
        if okRegion then
            canonicalRegion = tostring(canonical or canonicalRegion or "")
            rawRegion = tostring(raw or "")
        end
    end

    local safeBlocked = state.blockedSafeZone == true
    local safeReason = ""
    local safeLabels = ""
    local safeFn = EA and EA["EA_GetSafeZoneState"]
    if type(safeFn) == "function" and character and character ~= "" then
        local okSafe, safeState = pcall(safeFn, character)
        if okSafe and type(safeState) == "table" then
            safeBlocked = safeState.blocked == true
            safeReason = tostring(safeState.blockReason or "")
            if type(safeState.activeZones) == "table" then
                safeLabels = table.concat(safeState.activeZones, ",")
            end
        end
    end

    print(string.format(
        "[EnemyAmbush] %s: region=%s raw=%s safeZoneBlocked=%s reason=%s camp=%s combat=%s campExitGrace=%s/%ss cooldown=%s remaining=%ss pending=%s dangerMinutes=%s risk=%s%%",
        tostring(label or "state"),
        canonicalRegion ~= "" and canonicalRegion or "unknown",
        rawRegion ~= "" and rawRegion or "unknown",
        tostring(safeBlocked),
        safeReason ~= "" and safeReason or "none",
        tostring(state.inCamp == true),
        tostring(state.inCombat == true),
        tostring(state.campExitGraceActive == true),
        tostring(math.floor((tonumber(state.campExitGraceRemainingMs) or 0) / 1000)),
        tostring(state.cooldownActive == true),
        tostring(math.floor((tonumber(state.cooldownRemainingMs) or 0) / 1000)),
        tostring(state.pendingAmbushCount or 0),
        tostring(state.timeInDangerMinutes or 0),
        tostring(state.timeInDangerRiskPct or 0)
    ))
    if safeLabels ~= "" then
        print(string.format("[EnemyAmbush] %s safe zones: %s", tostring(label or "state"), safeLabels))
    end
end

local function EA_CountTableLike(value)
    local n = 0
...
local function EA_GetUninstallPrepCounts()
    ...
    if type(spawned) == "table" or type(spawned) == "userdata" then
        for id, _ in pairs(spawned) do
            if Osi and Osi.ObjectExists and Osi.ObjectExists(id) == 1 then
                activeTracked = activeTracked + 1
            end
        end
    end
    local pending = (type(EA_Pending) == "function") and EA_Pending() or nil
    local varsReady = type(vars) == "table" or type(vars) == "userdata"
    return {
        trackedRecords = EA_CountTableLike(spawned),
        activeTracked = activeTracked,
        pendingAmbushes = EA_CountTableLike(pending),
        hostileRetries = EA_CountTableLike(varsReady and vars.EA_PersistentHostileRetries or nil),
        championQueue = EA_CountTableLike(varsReady and vars.GuaranteedChampionQueue or nil),
        championArmed = EA_CountTableLike(varsReady and vars.GuaranteedChampionArmed or nil),
        delayedMirror = (varsReady and type(vars.EA_DelayedAmbushState) == "table" and vars.EA_DelayedAmbushState.timer) and 1 or 0,
        restDeferred = (varsReady and type(vars.EA_RestDeferredState) == "table" and vars.EA_RestDeferredState.timerName) and 1 or 0,
    }
end

local function EA_PrintUninstallPrepCounts(label, counts)
    print(string.format(
        "[EnemyAmbush] uninstallprep %s: trackedRecords=%d activeTracked=%d pending=%d hostileRetries=%d championQueue=%d championArmed=%d delayedMirror=%d restDeferred=%d",
        tostring(label or "dryrun"),
        tonumber(counts.trackedRecords) or 0,
        tonumber(counts.activeTracked) or 0,
        tonumber(counts.pendingAmbushes) or 0,
        tonumber(counts.hostileRetries) or 0,
        tonumber(counts.championQueue) or 0,
        tonumber(counts.championArmed) or 0,
        tonumber(counts.delayedMirror) or 0,
        tonumber(counts.restDeferred) or 0
    ))
end

local function EA_RunUninstallPrep(confirm)
    local before = EA_GetUninstallPrepCounts()
    if confirm ~= true then
        EA_PrintUninstallPrepCounts("dryrun", before)
        print("[EnemyAmbush] uninstallprep dryrun only. Use !ea_test uninstallprep confirm to disable future ambushes and clear Hunted active/pending runtime state.")
        return
    end
        ...
        for _, id in ipairs(keys) do
            if Osi and Osi.ObjectExists and Osi.ObjectExists(id) == 1 then
                if Osi.RequestDeleteTemporary then
                    pcall(Osi.RequestDeleteTemporary, id)
                end
                if Osi.ObjectExists and Osi.ObjectExists(id) == 1 and Osi.SetOnStage then
                    pcall(Osi.SetOnStage, id, 0)
                end
            end
            spawned[id] = nil
        end
    ...
    local pending = (type(EA_Pending) == "function") and EA_Pending() or nil
    if type(pending) == "table" or type(pending) == "userdata" then
    ...
    local vars = (type(EA_Vars) == "function") and EA_Vars() or nil
    if type(vars) == "table" or type(vars) == "userdata" then
        vars.EA_DelayedAmbushState = {}
        vars.EA_RestDeferredState = {}
        vars.EA_PersistentHostileRetries = {}
        vars.GuaranteedChampionQueue = {}
        vars.GuaranteedChampionArmed = {}
        vars.MCMSettings = vars.MCMSettings or {}
        vars.MCMSettings.MCM_EnableSummons = false
        vars.MCMSettings.MCM_EnableOnRest = false
        vars.MCMSettings.MCM_EnableTimeInDangerPressure = false
    end

    local applyFn = EA and EA["EA_ApplyOwnedRuntimeSettingsBatch"]
    if type(applyFn) == "function" then
        pcall(applyFn, {
            MCM_EnableSummons = false,
            MCM_EnableOnRest = false,
            MCM_EnableTimeInDangerPressure = false,
        }, { forceRefresh = true })
    end
    if MCM then
        local disabled = {
        ...
        local moduleUUID = tostring((EnemyAmbush and EnemyAmbush.ModuleUUID) or "96f24297-6ed9-455c-aaa1-ac9c358a8d35")
        for id, value in pairs(disabled) do
            if type(MCM.Set) == "function" then
                pcall(MCM.Set, id, value, moduleUUID, true)
            end
            if type(MCM.SetSetting) == "function" then
                pcall(MCM.SetSetting, id, value, moduleUUID, true)
            end
        end
    end
    ...
    local after = EA_GetUninstallPrepCounts()
    EA_PrintUninstallPrepCounts("confirm before", before)
    EA_PrintUninstallPrepCounts("confirm after", after)
    print("[EnemyAmbush] uninstallprep complete. BG3 can still complain when removing scripted mods from existing saves; this only clears Hunted-owned active/pending runtime state.")
end
...
elseif args[1] == "pressure" or args[1] == "state" then
    EA_PrintSupportAmbushState(player, args[1])
    return

elseif args[1] == "uninstallprep" then
    local mode = string.lower(tostring(args[2] or "dryrun"))
    if mode == "dryrun" or mode == "show" or mode == "status" then
        EA_RunUninstallPrep(false)
    elseif mode == "confirm" then
        EA_RunUninstallPrep(true)
    else
        print("[EnemyAmbush] Usage: !ea_test uninstallprep dryrun|confirm")
    end
    return
...
print("  !ea_test pressure|state - Print region, safe-zone, cooldown, pending ambush, and danger-pressure state")
print("  !ea_test uninstallprep dryrun|confirm - Report or clear Hunted active/pending runtime state before uninstall")
print("  !ea_test xpclones export - Export live template->stat source data for the XP-zero clone generator")
