...
function M.Build(deps)
    ...
    if EventsTimerFlow and type(EventsTimerFlow.Build) == "function" then
        local timerFlowDeps = {
            ...
            EA_GetCampExitAmbushGraceState = EA["EA_GetCampExitAmbushGraceState"],
            EA_UpdateCampExitAmbushGraceForCharacter = EA["EA_UpdateCampExitAmbushGraceForCharacter"],
    ...
    local function EA_HandleDelayedAmbushTimer(timer)
        ...
        if not runtimeReady then
            ...
            if pending[timer] then
                EA_TrackDelayedAmbushMirror(timer, pending[timer])
            else
                EA_ClearDelayedAmbushMirrorForTimer(timer)
            end
            return true
