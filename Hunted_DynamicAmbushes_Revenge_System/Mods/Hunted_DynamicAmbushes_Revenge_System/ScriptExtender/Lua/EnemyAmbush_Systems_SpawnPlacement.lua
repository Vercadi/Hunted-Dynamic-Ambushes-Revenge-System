...
local function EA_SpawnHostileNearPlayer_DoCreate(ctx)
    ...
    local safeZonePlacementRejects = 0
    ...
    local function EA_IsSpawnBlockedBySafeZone(candidate)
        local safeZoneFn = EA and EA["EA_GetObjectSafeZoneBlockState"]
        if type(safeZoneFn) ~= "function" or not candidate or candidate == "" then
            return false, "", nil
        end
        local okSafe, safeState = pcall(safeZoneFn, candidate)
        if okSafe and (type(safeState) == "table" or type(safeState) == "userdata") and safeState.blocked == true then
            return true, tostring(safeState.blockReason or "safe_zone_block"), safeState
        end
        return false, "", nil
    end
    ...
    local function EA_TryCreateOutOfSightPlacement(afterFindValid)
        ...
        for attempt = 1, 6 do
            ...
            if ok and guid and guid ~= "" then
                local rejectDeleteReason = "create_oos_probe_reject"
                local ex, ey, ez = SafeGetPosition(guid)
                if not ex then
                    ex, ey, ez = EA_TryGetPositionViaOsi(guid)
                end
                if ex then
                    local safeBlocked, safeReason, safeState = EA_IsSpawnBlockedBySafeZone(guid)
                    if safeBlocked then
                        safeZonePlacementRejects = safeZonePlacementRejects + 1
                        UpdateMetric("spawnSafeZoneRejected")
                        rejectDeleteReason = "spawn_safe_zone_block"
                        if EA_GetSettingBool("MCM_DebugMode", false) then
                            local activeZones = ""
                            if type(safeState) == "table" and type(safeState.activeZones) == "table" then
                                activeZones = table.concat(safeState.activeZones, ",")
                            end
                            DebugPrint(string.format("[Spawn] CreateOutOfSightAtDirection rejected (safe zone: %s %s)", tostring(safeReason), activeZones))
                        end
                    else
                ...
                EA_RequestDeleteWithRetry(guid, rejectDeleteReason)
        ...
        for i = 1, attempts do
            ...
            if validX and validX ~= 0 and (not badHeight) and (not tooClose) then
                ...
                if created and created ~= "" then
                    local safeBlocked, safeReason, safeState = EA_IsSpawnBlockedBySafeZone(created)
                    if safeBlocked then
                        safeZonePlacementRejects = safeZonePlacementRejects + 1
                        UpdateMetric("spawnSafeZoneRejected")
                        if EA_GetSettingBool("MCM_DebugMode", false) then
                            ...
                            DebugPrint(string.format("[Spawn] FindValidPosition spawn rejected (safe zone: %s %s)", tostring(safeReason), activeZones))
                        end
                        EA_RequestDeleteWithRetry(created, "spawn_safe_zone_block")
                        created = nil
                    end
                end
            ...
            if ok and created and created ~= "" then
                ...
                if safeBlocked then
                    ...
                    if EA_GetSettingBool("MCM_DebugMode", false) then
                        ...
                        DebugPrint(string.format("[Spawn] Fallback spawn rejected (safe zone: %s %s)", tostring(safeReason), activeZones))
                    end
                    EA_RequestDeleteWithRetry(created, "spawn_safe_zone_block")
                else
                    enemy = created
    ...
    if not enemy then
        ...
        if safeZonePlacementRejects > 0 then
            failureReason = "spawn_safe_zone_block"
        end
        UpdateMetric("spawnsFailed")
