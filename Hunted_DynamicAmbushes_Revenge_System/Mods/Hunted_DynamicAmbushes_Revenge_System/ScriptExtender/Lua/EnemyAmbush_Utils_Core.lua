...
local EA_SAVE_MAINTENANCE_VERSION = "1.0.6"
...
local function EA_CoreIsPersistentContainer(value)
...
function EA_RunSaveMaintenanceMigration(targetVersion)
    local vars = (type(EA_Vars) == "function") and EA_Vars() or nil
    if not EA_CoreIsPersistentContainer(vars) then
        return false, "vars_unavailable"
    end

    local version = tostring(targetVersion or EA_SAVE_MAINTENANCE_VERSION)
    if tostring(vars.EA_SaveMaintenanceVersion or "") == version then
        return false, "already_current"
    end

    local removedPending = 0
    local pending = vars.PersistentPendingAmbushes
    if EA_CoreIsPersistentContainer(pending) then
        local keys = {}
        ...
        for _, key in ipairs(keys) do
            ...
            local validRow = (rowType == "table" or rowType == "userdata")
                and type(key) == "string"
                and key ~= ""
                and type(row.kind) == "string"
                and row.kind ~= ""
    ...
    local removedSpawned = 0
    local spawned = vars.PersistentSpawnedEnemies
    if EA_CoreIsPersistentContainer(spawned) then
        local keys = {}
        for k, _ in pairs(spawned) do
            keys[#keys + 1] = k
        end
        for _, key in ipairs(keys) do
            local row = spawned[key]
            local rowType = type(row)
            local validRow = (rowType == "table" or rowType == "userdata")
                and type(key) == "string"
                and key ~= ""
            if not validRow then
                spawned[key] = nil
                removedSpawned = removedSpawned + 1
            end
        end
    end

    vars.EA_SaveMaintenanceVersion = version
    if type(EA_Dirty) == "function" then
        EA_Dirty()
    end
    if type(IsDebug) == "function" and IsDebug() then
        DebugPrint(string.format(
            "[Maintenance] save migration %s complete (pendingPruned=%d spawnedPruned=%d)",
            version,
            removedPending,
            removedSpawned
        ))
    ...
    return true, {
        version = version,
        pendingPruned = removedPending,
        spawnedPruned = removedSpawned,
    }
end
EA["EA_RunSaveMaintenanceMigration"] = EA_RunSaveMaintenanceMigration
...
function CheckVersion()
 ...
 pcall(EA_RunSaveMaintenanceMigration, EA_SAVE_MAINTENANCE_VERSION)
end
...
Ext.Vars.RegisterModVariable(ModuleUUID, "EA_SaveMaintenanceVersion", {
    Server = true, Client = false, WriteableOnServer = true, Persistent = true, SyncToClient = false
})
